...
void main(){
  ...
  else 
  {
	...
	vec2 pos_new;
	pos_new = pos_old + rdx * timestep * dir_new / wh_fluid;
	
	pos_new.y -= 0.0015;
	float dist = length(pos_old-pos_new);
	if (dist < 0.002) pos_new = vec2(-1,0);
	// pos_new = vec2(0,0);
	
	float clipDist = 0.01;
	if ((pos_new.x < clipDist) || (pos_new.y <clipDist) ||
		(pos_new.x > 1-clipDist) || (pos_new.y > 1-clipDist))	{
		pos_new = vec2(-1,0);
	}
